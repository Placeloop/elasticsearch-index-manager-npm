...
var _recursive_delete = function(directory) {
  return promise.promisify(rimraf)(directory).then(
    function() {
      return true
    }
  )
...
var _fold_directory = function(directory, folder, initial) {
  return promise.promisify(fs.readdir, fs)(directory).then(
    function(content) {
...
var _write_file = function(file, content) {
  return promise.promisify(fs.writeFile, fs)(file, content).then(
    function() {
      return true
    }
  )
