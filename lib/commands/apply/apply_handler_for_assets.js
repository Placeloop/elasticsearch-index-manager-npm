...
var _put_assets = function (es_setup, asset_kind, configs, assets, putter) {
  ...
  var promise_chain = promise.resolve().then(
    function() {
      console.log(console_colors.FGWHITE, "Starting putting " + asset_kind + " ...")
    }
  )
  ...
  promise_chain = promise_chain.then(
    function(post_actions) {
      console.log(console_colors.FGWHITE, "Putting " + asset_kind + " done!\n")
...
var _put_config = function(esclient, promise_chain, config_with_name, config_name, next_config) {
  return promise_chain.then(
    function() {
      ...
      return esclient.indices.getTemplate({ name : config_name, ignore: [404] }).then(
        function(previous_config) {
          ...
          return esclient.indices.putTemplate({ name : config_name, create : false, body : next_config }).then(
            function() {
...
var _put_query_template = function(esclient, promise_chain, config_with_name, query_template_name, next_query_template) {
  return promise_chain.then(
    function() {
      ...
      return esclient.getTemplate({ id : query_template_name, ignore: [404] }).then(
        function(previous_query_template) {
          ...
          return esclient.putTemplate({ id : query_template_name, body : next_query_template }).then(
            function() {
