      ...
      if(found_index_name === "") {
       ...
       return esclient.indices.create({ index : index_first_version }).then(
          function(create_result) {
            ...
            return { post_action : "noop", current_version : index_first_version }
          }
        )
        ...
        promise_chain = promise_chain.then(
          function(at_least_one_mapping_update_fail) {
            if(at_least_one_mapping_update_fail) {
              ...
              return { post_action : "reindex", current_version : found_index_version }
            } else {
              console.log(console_colors.FGWHITE,"Index '"+found_index_name+"' mappings updated")  
              return { post_action : "update", current_version : found_index_version }
            }
          }
        )
        ...
        return esclient.indices.create({ index : index_name }).then(
          ...
          function(create_result) {
            return esclient.indices.deleteAlias({ index: found_index_name, name : index_writer }).then(
              function(delete_alias_result) {
                console.log(console_colors.FGWHITE, " Index writer removed from: '"+found_index_name+"'")
                return { post_action : "reindex", : previous_version : "", current_version : "" }            
              }
            )
          }
        )
