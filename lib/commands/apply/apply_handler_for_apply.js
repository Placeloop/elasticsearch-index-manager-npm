...
module.exports.apply_index_config = function (es_setup, config_name, config, create_next_version) {
  ...
  return esclient.indices.get({ index : index_reader }, { ignore: [404] }).then(
    function(index_config_data) {
      index_config_data = index_config_data.body || index_config_data
      ...
      if(found_index_name === "") {
       ...
       return esclient.indices.create({ index : index_first_version }).then(
        ...
        promise_chain = _apply_mapping_to_config(promise_chain, esclient, found_index_name, config.template.mappings)
...
var _apply_mapping_to_config = function(promise_chain, esclient, found_index_name, config) {
  return promise_chain.then(
    function(previous_mapping_update_fail) {
      ...
      return esclient.indices.putMapping({ index : found_index_name, body : config }).then(
        function(mapping_update_result) {
          console.log(console_colors.FGWHITE, "     Type '"+found_index_name+"' updated!")
          return promise.resolve(false)
        }
      ).catch(
        function(error) {
          console.log(console_colors.FGYELLOW, "     Type '"+found_index_name+"' not updated! Reindex is required. Reason = ", error)
