...
module.exports.put_and_config = function(params, generated_assets) {
  es_setup = _setup_esclient(params)
  return _put_assets_handler(es_setup, generated_assets).then(
    function() {
      return _apply_configs(es_setup, generated_assets.configs, constants.NOOP_POST_ACTION)
    }
  ).then(
    es_setup.esclient_releaser_on_success
  ).catch(
...
var _setup_esclient = function(params) {
  var auth = {
    username: params.basic_auth.split(':')[0],
    password: params.basic_auth.split(':')[1]
  }
  var esclient = new elasticsearch.Client({
    node: params.base_url,
    auth: auth,
    log: 'info',
    ssl: {
      rejectUnauthorized: false,
      // See this bug https://issueexplorer.com/issue/parse-community/node-apn/75
      pfx: []
    },
  ...
  return promise_chain.then(
    ...
    function() {
      ...
      function(update_index_config_result) {
        return _assert_open_index(es_setup, config_name, config).then(
          function(){
            return update_index_config_result
          }
        )
      }
  ).then(
...
var _assert_open_index = function(es_setup, config_name, config) {
  return promise.resolve().then(
    function() {
      console.log(console_colors.FGWHITE, "  Asserting '"+config_name+"' is open ...")
    }
  ).then(
    function() {
      ...
      return esclient.indices.open({ index : index_reader })
    }
  ).then(
    function() {
      console.log(console_colors.FGWHITE, "  '"+config_name+"' is open!")
    }
  ).catch(
    function(error) {
      console.log(console_colors.FGYELLOW, "  '"+config_name+"' not open due to : ", error)
      return promise.resolve()
    }
  )
}
